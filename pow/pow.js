...
function load_pow_files() {
  // TODO: attach source pow_file and base_dir to fns
  const pow_files = get_pow_files(pow.cwd);
  const promises = pow_files.map((file) =>
    import(file)
      .then((mod) => {
        pow.log.debug(`Loaded pow_file ${file}`);
        for (const full_key in mod) {
          ...
          if (
            ...
            fn.pow_file = file;
            pow.fns[key] = fn;
            pow.log.debug(`Registered ${full_key}`);
          }
        }
      })
      .catch((err) => {
        pow.log.error(`Failed to load ${file}`);
        throw err;
      })
  );
